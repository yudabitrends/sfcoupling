#!/usr/bin/env python3
"""
Phase 1 W3 precursor: aggregate 20 pre-extracted UK Biobank feature chunks
(chunk_0{00..19}.npz) into the run_rscm.py-expected
dataset1_*/dataset2_* aligned-features layout.

Each chunk: 591 subjects × (99 GM + 1378 FNC + age + sex + eid + status).
Total: 11,820 subjects — 11× the UKB 1,079 smoke, 31% of full UKB.

Split: 80% DS1 (9,456) with CV 70/15/15 train/val/test; 20% DS2 (2,364)
...
Paths are configurable via environment variables (defaults are repo-relative):
  ...
  ALIGNED_DIR    output aligned_features dir  (default: data/aligned_features_ukb11820)
  SPLITS_DIR     output splits dir            (default: data/splits_ukb11820)
...
DST_AF = Path(os.environ.get("ALIGNED_DIR", "data/aligned_features_ukb11820"))
DST_SP = Path(os.environ.get("SPLITS_DIR", "data/splits_ukb11820"))
...
def main():
    ...
    for c in chunks:
        d = np.load(c)  # no allow_pickle — all arrays are numeric or unicode
        ...
        print(f"  {c.name}: {int(keep.sum())}/{len(keep)} kept")
